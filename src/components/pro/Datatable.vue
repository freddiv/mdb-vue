    ...
    <div v-if="pagination" class="row">
      <div class="col-sm-12 col-md-5">
        ...
        <div v-if="filter">
          ...
          <mdb-select class="mt-0 w-50 d-inline-block" @getValue="updateSearch" :options="filterOptions" />
        </div>
      </div>
...
const Datatable = {
  ...
  beforeMount() {
    ...
    if (this.filter) {
      this.rows.map(row => {
        this.filterArray.push(row[this.filter]);
      });
      this.filterArray.sort();
      this.filteredArray = this.filterArray.filter(function(elem, index, self) {
